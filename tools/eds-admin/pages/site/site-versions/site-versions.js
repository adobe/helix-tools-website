...
export class SiteVersions extends LitElement {
  static properties = {
    _org: { state: true },
    _site: { state: true },
  ...
  async _openVersion(id) {
    if (!this._org || !this._site || !id) return;
    this._selectedId = id;
    this._versionDetail = null;
    this._prevVersionDetail = null;
  ...
  async _handleDeleteVersion() {
    ...
    this._error = '';
    const { status, error } = await deleteVersion(this._org, this._site, this._deleteConfirmId);
    this._deletingVersion = false;
  ...
  render() {
    return html`
      <div class="page">
        <div>
          <h1 class="page-title">Site Config Versions</h1>
          <p class="page-subtitle">
            Version history for <strong>${this._org}/${this._site}</strong>. You can view or restore any previous version.
          </p>
        </div>
...
customElements.define('site-versions', SiteVersions);
