...
import { fetchApiKeys, createApiKey, deleteApiKey } from '../../../services/adminApi.js';
...
export class SiteApiKeys extends CrudPage {
  static properties = {
    ...CrudPage.properties,
  ...
  get _pageTitle() { return 'Site API Keys'; }
  get _emptyMessage() { return 'No API keys configured for this site.'; }
  get _createButtonLabel() { return 'Create API Key'; }
  get _deleteHeadline() { return 'Delete API Key'; }

  _renderPageSubtitle() {
    return html`API keys for programmatic access to ${this._org} / ${this._site}. Keep these secure.`;
  }

  _renderDeleteMessage() {
    return html`<p>Are you sure you want to delete this API key? Any integrations using it will stop working immediately.</p>`;
  }

  async _fetchItems() { return fetchApiKeys(this._org, this._site); }
  async _createItem(body) { return createApiKey(this._org, this._site, body); }
  async _deleteItem(id) { return deleteApiKey(this._org, this._site, id); }
    ...
    return html`
      <eds-dialog open headline="Create API Key" size="m"
        ...
        <div style="display:flex; flex-direction:column; gap:8px;">
          <div>
            <label class="field-label" for="api-key-description">Description (optional)</label>
            <eds-textfield id="api-key-description"
              .value=${this._newKeyDescription}
              @input=${(e) => { this._newKeyDescription = (e.detail?.value ?? e.target?.value ?? ''); }}
            ></eds-textfield>
          </div>
          <div>
            <eds-picker id="api-key-role" .value=${this._newKeyRole} label="Role"
...
customElements.define('site-api-keys', SiteApiKeys);
