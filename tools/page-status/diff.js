...
async function fetchJobDetails(org, site, jobId) {
  const jobPath = `/job/${org}/${site}/main/status/${jobId}`;
...
async function loadPageDiff(page) {
  ...
  // Build admin API paths to fetch markdown content
  const fetchPath = path.endsWith('/') ? `${path}index.md` : `${path}.md`;
  const previewApiPath = `/preview/${currentOrg}/${currentSite}/main${fetchPath}`;
  const liveApiPath = `/live/${currentOrg}/${currentSite}/main${fetchPath}`;
  ...
  try {
    ...
    const [previewResult, liveResult] = await Promise.all([
      adminFetch(previewApiPath).then(async (res) => ({
        content: res.ok ? await res.text() : null,
        status: res.status,
      })),
      adminFetch(liveApiPath).then(async (res) => ({
        content: res.ok ? await res.text() : null,
        status: res.status,
      })),
    ]);
