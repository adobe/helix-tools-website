...
This project is the website for https://tools.aem.live/, which provides administrative and development tools used with AEM Edge Delivery Services (EDS).

The project is based on the https://github.com/adobe/aem-boilerplate/ project and set up as a new project. You are expected to follow the coding style and practices established in the boilerplate, but add functionality according to the needs of the site currently developed.
...
- Install dependencies: `npm install` (also runs postinstall to generate `vendor/` files)
...
├── tools/           # Tool-specific implementations
    └── {toolname}/   - Individual tool directory
...
├── scripts/         # JavaScript libraries and utilities
    ...
    └── vendor.mjs       # Postinstall script that generates vendor/ (not served)
├── vendor/          # Vendored browser dependencies (generated by postinstall, committed to git)
├── fonts/           # Web fonts
├── icons/           # SVG icons
...
Runtime JavaScript dependencies are managed through npm and vendored into `vendor/` as browser-ready ES modules. This eliminates runtime CDN dependencies and provides lockfile protection.

- `scripts/vendor.mjs` runs as a postinstall hook after `npm install`, copying or bundling dependencies from `node_modules/` into `vendor/`
- Vendor files are committed to git because EDS has no server-side build step
- The script is idempotent — it hashes `package-lock.json` and skips if nothing changed
- Dependabot tracks versions with a cooldown period; when it bumps a dependency, run `npm install` and commit the regenerated `vendor/` files
- Never load JavaScript libraries from external CDNs at runtime — always reference `/vendor/` paths in import maps and dynamic imports
...
- **Production Preview**: `https://main--helix-tools-website--adobe.aem.page/`
- **Production Live**: `https://main--helix-tools-website--adobe.aem.live/`
- **Feature Preview**: `https://{branch}--helix-tools-website--adobe.aem.page/`
...
4. Open a pull request to merge changes to `main`
   1. in the PR description, include a link to `https://{branch}--helix-tools-website--adobe.aem.page/{path}` with a path to a file that illustrates the change you've made. This is the same path you have been testing with locally. WITHOUT THIS YOUR PR WILL BE REJECTED
...
### Adding A New Tool
1. Create directory: `tools/{toolname}/`
2. Add `{toolname}.js`, `{toolname}.css`, and `index.html`
3. Implement tool logic in JavaScript
4. Style the tool with scoped CSS selectors
...
1. Add or update the package version in `package.json` under `dependencies`
2. If it is a new dependency, add a copy or esbuild bundle entry in `scripts/vendor.mjs`
3. Run `npm install` — the postinstall hook regenerates `vendor/` files
4. Update import maps in HTML files or `import` statements in JS files to reference the `/vendor/` path
5. Commit the updated `vendor/` files along with the code changes
